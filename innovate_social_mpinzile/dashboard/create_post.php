    ...
    if (empty($title) || empty($description) || empty($image['name'])) {
        ...
        if (!in_array($imageExtension, $allowedExtensions)) {
            ...
            if (move_uploaded_file($image['tmp_name'], $uploadPath)) {
                ...
                $stmt->bind_param("isssss", $user_id, $title, $description, $uploadPath, $postTime, $postDate);

                if ($stmt->execute()) {
                    $successMsg = "Post created successfully!";
                    header('Refresh:3;url=../dashboard');
                } else {
                    $errorMsg = "Error creating post: " . $stmt->error;
                }
                $stmt->close();
            } else {
...
<header class="bg-primary text-white text-center py-3">
    ...
    <p class="lead">Create and share your posts with the community.</p>
</header>
